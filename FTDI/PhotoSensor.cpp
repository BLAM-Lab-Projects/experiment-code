...
int PhotoSensor::InitSensor(int i, FT_HANDLE *ftHandle, int devMode, UCHAR mask)
{
	DWORD numDevs;
	FT_STATUS status;
	//DWORD Flags;
...
bool PhotoSensor::CloseSensor(FT_HANDLE ftHandle, int devMode)
{
	FT_STATUS status;
...
int PhotoSensor::GetSensorData(FT_HANDLE ftHandle)
{
...
int PhotoSensor::GetSensorBitBang(FT_HANDLE ftHandle,int bit)
{
...
int PhotoSensor::SetSensorBitBang(FT_HANDLE ftHandle,UCHAR mask, int bit, int value)
{
